...
function petsHandle(req: any, res: any) {
  ...
  async function createUser(data: any) {
    const { petsId, usersId, nome, raca, genero, idade, sexo, castrado } = data;
    try {
      const { data: petData } = await axios.post(
        `${process.env.NEST_API_HOST}/pets`,
        {
          nome,
          raca,
          genero,
      ...
      await axios.post(`${process.env.NEST_API_HOST}/users/pets`, {
        petsId: petData.id,
        usersId: usersId,
      }),
        res.status(200).send();
